...
class Board:
    ...
    def add_bonus(self):
        ...
        for i in TRIPLE_WORD_SCORE:
            self.grid[i[0]][i[1]] = Cell(None,None,3,'word')
        for j in TRIPLE_LETTER_SCORE:
            self.grid[j[0]][j[1]] = Cell(None,None,3,'letter')
        for k in DOUBLE_WORD_SCORE:
            self.grid[k[0]][k[1]] = Cell(None,None,2,'word')
        for l in DOUBLE_LETTER_SCORE:
            self.grid[l[0]][l[1]] = Cell(None,None,2,'letter')
